...
class Reservation_buffers extends gui {
    ...
    public function view_cart_element(){
        ...
        return '<tr>
                    <td align="center"><a href="./delete_buffer_element.php?rbid='.$this->id.'"><button>X</button></a></td>
                ...
                '<tr>
                    <td colspan="9" align="center"><a target="_blank" href="./pay.php" onclick="window.opener.location.reload();"><button class="buy-button">'.locales::$text[tickets_buy].'</button></a></td>
                 </tr>';
    }
    ...
    public function delete_buffer_element() {
        if(isset($_SESSION['buffer_id'])) {
            $rbid = (int)$_GET['rbid'];

            $resb = new Reservation_buffer();

            $resb->query($result, "DELETE FROM reservation_buffer WHERE id = '$rbid' AND buffer_id LIKE '{$_SESSION['buffer_id']}'");
        }
