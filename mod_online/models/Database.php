...
class Database {
    ...
    public function find(array $params){
        ...
        $this->Query($result, $query); //print '<br />'.$query; exit;
        if(isset($params['id'])){
    ...
    protected function separateInPages ($query,$page_size){
        ...
        if($size>0){
            ...
            if($page>1) $links.='<a href="'.$_SERVER[PHP_SELF].'?pg=1'.$argum.'">&laquo;</a> <a href="'.$_SERVER[PHP_SELF].'?pg='.($page-1).$argum.'">&larr;</a>';
            ...
            $links.=' './*$page.*/' от '.$count.' ';
            if($page<$count) $links.='<a href="'.$_SERVER[PHP_SELF].'?pg='.($page+1).$argum.'">&rarr;</a> <a href="'.$_SERVER[PHP_SELF].'?pg='.$count.$argum.'">&raquo;</a>';
            $links.='</div>';
