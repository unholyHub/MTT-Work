...
class Invoices extends gui{
    ...
    public function show_invoice_payement_details($operation, $message){
        $messageHeader = '';
        if($operation == '00') {
            $messageHeader = locales::$text['ok_msg'];
        } else {
            $messageHeader = locales::$text['no_payment'];
        }
        
        $body = '<h3 align="center">'.  $messageHeader . '</h3>'.
                $message;
    ...
    public function invoice_details(){
        ...
        foreach($elements as $element){
            ...
            $emails[$reservation->contact_email][] = $element->reservation_id;
    ...
    public function send_invioice_details($details){
        ...
        foreach($emails as $email => $val){
            $this->send_email($email, $message, $val);
        }
    }

    private function send_email($email,$message, $rids){
        require_once '../mod_swift/lib/swift_required.php';
        
        $transport = Swift_SmtpTransport::newInstance();

        $mailer = Swift_Mailer::newInstance($transport);
        $message = Swift_Message::newInstance('Online Ticket System')
          ->setFrom(Env::SalesMail)
          ->setTo($email)
          ->setContentType('text/html')
          ->setBody($message);
        
        foreach($rids as $rid) {
            $attachment = Swift_Attachment::newInstance(file_get_contents("http://{$_SERVER['HTTP_HOST']}".str_ireplace($_SERVER['DOCUMENT_ROOT'], '', dirname(__DIR__))."/ticket2pdf.php?rid={$rid}"), 'Online_ticket('.$rid.').pdf');  
